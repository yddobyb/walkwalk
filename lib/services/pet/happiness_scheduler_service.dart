...
class HappinessSchedulerService {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _isInitialized = true;
      debugPrint('HappinessSchedulerService - Initialized successfully');
    } catch (e) {
  ...
  Future<void> _applyMissedDailyDecay() async {
    try {
      ...
      debugPrint('HappinessSchedulerService - Applied missed daily decay check');
    } catch (e) {
      debugPrint('HappinessSchedulerService - Error applying missed decay: $e');
    }
  }
  ...
  void _scheduleNextDecay() {
    ...
    debugPrint('HappinessSchedulerService - Next decay scheduled in ${timeUntilMidnight.inHours}h ${timeUntilMidnight.inMinutes % 60}m');
  }
  ...
  Future<void> _applyDailyDecay() async {
    try {
      ...
      if (updatedPet != null) {
        debugPrint('HappinessSchedulerService - Daily decay applied. Happiness: ${updatedPet.happiness}');

        // 행복도가 낮아졌다는 알림을 보낼 수 있음 (추후 구현)
        if (updatedPet.happiness <= AppConstants.minHappiness + 10) {
          debugPrint('HappinessSchedulerService - Pet happiness is getting low!');
        }
      }
    } catch (e) {
      debugPrint('HappinessSchedulerService - Error applying daily decay: $e');
    }
  }
